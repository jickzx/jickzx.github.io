// Token is fetched securely from the server — never hardcoded or exposed in source
let ionToken = null;
try {
  const res = await fetch('/api/cesium-token');
  if (res.ok) ({ token: ionToken } = await res.json());
} catch (e) { console.warn('Cesium token unavailable:', e); }
